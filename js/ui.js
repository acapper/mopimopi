...
function putValue(arr, c) {
	for (var i in arr) {
		if (init[c][arr[i]] == undefined) init[c][arr[i]] = Mopi2[c][arr[i]];
	}
}
function SubOption() {
	for (var i in init.q) {
...
function initOverlay(val) {
	...
	if (lastCombat == null) {
		...
		for (var i in l.Notice) {
			if (lang == "KR") $("#" + i).html(l.Notice[i][lang]);
			else $("#" + i).html(l.Notice[i].EN);
		}
	} else {
...
$(".dropdown ul").scroll(function () {
	if ($(this)[0].childElementCount > 5) {
		if ($(this).scrollTop() == 0) {
			...
			if (
				$(this)[0].scrollHeight <=
				Math.ceil($(this).height()) + Math.ceil($(this).scrollTop())
			) {
				$("#gradD").fadeOut(0);
				$("#gradU").fadeIn(0);
			} else {
...
function resizeWindow(flag) {
	if (
		init.q.view24 &&
		lastCombat != null &&
		lastCombat.partys >= init.q.view24_Number
	) {
		if (window.innerWidth % init.Range.size24TableSlice != 0) {
			var w = window.innerWidth % init.Range.size24TableSlice;
			$(".rRow .rCell").css(
				"width",
				parseFloat(100 / init.Range.size24TableSlice) + "%"
			);
			for (var i = 0; i < w; i++)
				$(".rRow .rCell:nth-child(" + parseInt(i + 1) + ")").css(
					"width",
					"-webkit-calc(" +
						parseFloat(100 / init.Range.size24TableSlice) +
						"%" +
	...
	if (flag == "settings") {
		$(".scrollArea").css(
			"height",
			"-webkit-calc(100vh - " +
				$("nav[name=settings]")[0].offsetHeight +
				"px - " +
				$(".previewArea")[0].offsetHeight +
				"px - " +
				$(".tabArea")[0].offsetHeight +
				"px)"
		);
		...
		$("#HISTORYBody").css(
			"height",
			"-webkit-calc(100vh - " +
				$("nav[name=history]")[0].offsetHeight +
				"px - " +
				$("#HISTORYHeader")[0].offsetHeight +
				"px)"
		);
	} else {
		$("#update").css(
			"height",
			"-webkit-calc(100vh - " +
				$("nav[name=main]")[0].offsetHeight +
				"px - " +
				$("#strong")[0].offsetHeight +
				"px - " +
				$("#tip")[0].offsetHeight +
				"px)"
		);
...
$("nav[name=main] div[name=More]").on({
	mouseover: function () {
		if ($(this).attr("name") == "More") {
			...
			else $(this).parent().find("div").fadeIn(150);
		}
	},
});
$("nav[name=main] .btn_wrap").on({
	mouseleave: function () {
		...
		for (var i in arr) {
			...
			else $("nav[name=main] div[name=" + arr[i] + "]").fadeOut(150);
		}
	},
});
$(".btn_wrap div").on({
	...
	if (init.q.autoHide && view != "settings") {
		...
		let createTimeout = function (divName) {
			...
			if (view == "history") $("div[name=history]").fadeIn(0);
				...
				divRef.fadeIn(0);
				if (mainNavRef[0]) mainNavRef[0]?.setAttribute("style", mainBg);
			}
...
function ctrlPreview(flag) {
	if (flag == true) {
		init.q.preview = 1;
		if ($(".previewArea")[0].innerHTML == "") createDOM("preview");
		else ui();

		if (init.q.view24_Number == 1) toggleRaidMode(1);
	} else {
		init.q.preview24 = 0;
		init.q.preview = 0;
		$(".previewArea").html("");
...
function togglePreview() {
	if (init.q.preview == 1) ctrlPreview(1);
	else ctrlPreview(0);
}
...
function createKeys() {
	...
	for (var i in ui) {
		if (i != "Color" && i != "Range" && i != "q") delete ui[i];
	}
	for (var i in del) delete ui.q[del[i]];
...
function applyKeys(data) {
	try {
		...
		if (e instanceof SyntaxError) {
			if (data == "") callToast("noInput", 500, 3000);
...
function loadPreview(el_img) {
	...
	if (
		...
		return;
	} else loadPreviewFile(el_img.files[0]);
}
...
function liReload() {
			...
			if ($(event.target)[0].tagName == "A") {
				...
				if (!$(this).find(".switch").hasClass("hover")) {
					...
					if (id.indexOf("DPS-") > -1 || id.indexOf("HPS-") > -1) {
						...
						init.Order[id.split("-")[0]].splice(
							init.Order[id.split("-")[0]].indexOf(id.split("-")[1]),
							1
						);
					} else init.q[id] = 0;
				}
				if (
					(id == "pets" || id == "hideName" || id == "view24") &&
					view != "settings"
				) {
				...
				if ($(this).find(":radio").attr("name").indexOf("align") > -1) {
					...
					init.q[$(this).find(":radio").attr("name")] = $(this)
						.find(":radio")
						.val();
				if ($(this).find(":radio").attr("name") == "view24_Number") {
					$("li#" + $(this).find(":radio").attr("name") + " .gVal").text(
						l.raid.tab_general.inner.view24_Number.msg[lang].replace(
							"★",
							$(this).text()
						)
					);
					if (init.q.view24_Number == 1) toggleRaidMode(1);
					else toggleRaidMode(0);
				} else
					$("li#" + $(this).find(":radio").attr("name") + " .gVal").text(
						$(this).text()
					);
				if ($(this).find(":radio").attr("name") == "Lang") {
					...
					$("nav[name=settings]")
						.find("table td")
						.text(l.NAV.settings.tt[lang]);
					$("#preview24 td:first-child").text(
						l.raid.tab_general.inner.view24_Number.tt[lang]
					);
				...
				if ($(this).hasClass("radio")) {
					...
					if (id.indexOf("alignHeader") > -1 || id.indexOf("alignBody") > -1) {
						...
						if (id == "palette") var obj = l.Graph[init.q.palette][id];
						...
						} else var obj = l[$(this).attr("p")][id];
						createDOM("dr_radio", obj, id);
					}
				} else button($(this).attr("id"));
				return;
			}
		});
	$("#preview24")
	...
	$(".gTitle.sendBtn")
		...
		.bind("click", function () {
			...
			if (key != "" && val != "" && key != undefined && val != undefined) {
				...
				liReload();
				callToast("ok", 500, 3000);
			} else callToast("noInput", 500, 3000);
			ui();
			return;
		});
	$("#in_share")
			...
			if (e.keyCode == 13) {
				...
				if (id == "abbOld" || id == "abbNew") {
					...
					if (key != "" && val == "") {
						...
						init.Alias[key] = val;
						$("ul.remove").append(
							createElement("li_remove_list", null, key, val)
						);
						$("#in_abbOld,#in_abbNew").val("");
						liReload();
						callToast("ok", 500, 3000);
					...
					id.indexOf("f") > -1 ||
					category.indexOf("headerText") > -1
				) {
					if (input != "") {
						if (category.indexOf("headerText") > -1) {
							init.ColData[id].tt = input;
							update(previewDPS, previewHPS);
						} else init.q[id] = input;
						$("[name=" + id + "]")
							.find(".gVal")
							.text(input);
						$(this).blur();
	...
	$("input[type=range]")
		...
		.bind("input", function (e) {
			...
			if (id.indexOf("sizeWidth") > -1)
				init.ColData[id.replace("sizeWidth", "")].width = parseInt(
					$(this).val()
				);
			else if (id.indexOf("sizePadding") > -1)
				init.ColData[id.replace("sizePadding", "")].padding = parseInt(
					$(this).val()
				);
			else init.Range[id] = parseInt($(this).val());
			var unit = "%";
			...
			} else if (id == "autoHideTime") unit = "min";
			$(this)
				.parents("li")
				.find(".gVal")
				.html($(this).val() + unit);
	...
	$(".UBtn")
		...
		.bind("click", function () {
			...
			init.Order[$(this).parents(".listBox").attr("flag")] = getOrder(
				".listBox",
				$(".scrollArea")[0]
			);
	...
	switch (id) {
		...
		case "Back":
			if (direction == "main") {
				...
				if (lastCombat != null) $("[name=main]").fadeIn(0);
		...
		case "Settings":
			...
			$("nav[name=settings] [name=Back]").attr(
				"onclick",
				"button('Back', 'main')"
			);
			$("[name=main], [name=notice]").fadeOut(0);
		...
		case "Overlay":
		case "Tool":
			if (id == "Data") ctrlPreview(1);
			else ctrlPreview(0);
			if (direction != "Back") sVal.old = sVal.now;
			$(".scrollArea").scrollTop(0);
			...
			if (id == "Tool") $("#backup .gVal").html(init.q.backupDate);
			resizeWindow(view);
			break;
		case "font":
		case "custom":
			if (id == "font") ctrlPreview(1);
			else ctrlPreview(0);
			sVal.pre = sVal.now;
			$(".scrollArea").scrollTop(0);
		...
		case "abbset":
			...
			if (id == "abbset") {
				...
				$("nav[name=settings]")
					.find("table td")
					.text(l[l.back[id]]["tab_mhh"]["inner"][id].tt[lang]);
				createDOM("page", l[id], id);
				...
				$("nav[name=settings]")
					.find("table td")
					.text(l[l.back[id]]["tab_general"]["inner"][id].tt[lang]);
				button(createDOM("tab", l[id]), id);
			}
			resizeWindow(view);
			break;
		...
		case "HPSfilter":
			...
			createDOM(
				"dr_checkbox",
				l.format["tab_" + id.split("f")[0]].inner[id].dr
			);
			break;
	}
}
		...
		if (obj.DPS == 1 || obj.HPS == 1) {
			if (tab == "tab_width" && i != "name") {
				ul.append(
					'<li id="sizeWidth' +
						i +
						'" class="li_box"><table><tr><td rowspan="2" class="gIcon"><i class="material-icons">arrow_right</i></td><td class="gTitle">' +
						obj.tt +
						'<font class="ex">　❙ ' +
						d[i].tt +
						'</font></td><td rowspan="2" style="padding:0 1.4rem"><input class="shadow" type="range" min="0" max="200" value="' +
						obj.width +
						'"></td></tr><tr><td class="gVal ac">' +
						obj.width +
						"px</td></tr></table></li>"
				);
				...
				ul.append(
					'<li id="sizePadding' +
						...
						'<font class="ex">　❙ ' +
						d[i].tt +
						'</font></td><td rowspan="2" style="padding:0 1.4rem"><input class="shadow" type="range" min="0" max="10" value="' +
						obj.padding +
						'"></td></tr><tr><td class="gVal ac">' +
						obj.padding +
						"px</td></tr></table></li>"
				...
				ul.append(
					'<li id="alignHeader_' +
						...
						'" class="radio" p="' +
						obj.p +
						'"><table><tr><td rowspan="2" class="gIcon"><i class="material-icons">arrow_right</i></td><td class="gTitle">' +
						obj.tt +
						'<font class="ex">　❙ ' +
						d[i].tt +
						" ❯ " +
						d.header[lang] +
						'</font></td></tr><tr><td class="gVal ac ">' +
						d.direction[obj.alignHeader][lang] +
						"</td></tr></table></li>"
				);
				ul.append(
					'<li id="alignBody_' +
						...
						d.body[lang] +
						'</font></td></tr><tr><td class="gVal ac ">' +
						d.direction[obj.alignBody][lang] +
						"</td></tr></table></li>"
				);
			} else if (tab == "tab_title") {
				if (flag == false) {
					ul.append(
						'<li class="li_box"><table><tr><td rowspan="2" class="gIcon"><i class="material-icons">priority_high</i></td><td class="gTitle">' +
							d.caution_tt[lang] +
							'</td></tr><tr><td class="gVal ex" style="padding-right:1.4rem; text-align:justify">' +
							d.caution_m1[lang] +
							"</td></tr></table></li>"
					);
				}
				...
				ul.append(
					'<li class="li_text" style="border:0"><table><tr><td rowspan="2" class="gIcon"><i class="material-icons">text_fields</i></td><td style="width:100%;"><div class="inputBox"><input class="inputEff" type="text" placeholder="' +
						d.headerText[lang] +
						'" id="headerText_' +
						i +
						'"><span class="focus-border"></span></div></td><td rowspan="2" class="gIcon ft sendBtn"><i class="material-icons">send</i></td></tr></table></li>'
				);
			}
			flag = true;
		}
...
function createCellPageDropDown(id, category) {
	...
	for (var i in d.direction) {
		if (init.ColData[id][category] == i) on = "hover";
		else on = "";
		html +=
			"<li>" +
			d.direction[i][lang] +
...
function createDOM(type, obj, id) {
	...
	switch (type) {
		...
		case "dr_radio":
			...
			break;
		case "dr_checkbox":
			for (var i in obj) html += createElement(type, obj[i], i);
			$(".dropdown ul").html(html);
			resizeDropdown();
			break;
		case "page":
			if (id != undefined && id.indexOf("tab_") > -1) $(".scrollArea").html("");
		...
		case "list_remove":
			...
			ul.append(
				'<li class="li_box"><table><tr><td rowspan="2" class="gIcon"><i class="material-icons">arrow_right</i></td><td class="gTitle">' +
					l.abbset.in_abbOld.m[lang].split("(")[0] +
					'</td></tr><tr><td class="gVal ac">' +
					l.abbset.in_abbNew.m[lang] +
					"</td></tr></table></li>"
			);
			for (var i in obj)
	...
	switch (type) {
		case "dr_checkbox":
			...
			return (
				...
				">" +
				obj.tt[lang] +
				'<input type="checkbox"/><div class="switch ' +
				on +
				'"><div class="toggle"></div></div></li>'
			);
		case "dr_radio":
		...
		case "tab_btn":
			return (
				...
				'" p="' +
				obj.p +
				'" class="tab_box" style="width:' +
				obj.w +
				'%"><div class="tab_title">' +
				obj.tt[lang] +
				'</div><div class="tab_underBar"></div></div>'
			);
		case "li_2line":
			return (
				'<li id="' +
				id +
				'"><table><tr><td rowspan="2" class="gIcon"><i class="material-icons">' +
				obj.i +
				'</i></td><td class="gTitle">' +
				obj.tt[lang] +
				'</td></tr><tr><td class="gVal ac">' +
				obj.m[lang] +
				"</td></tr></table></li>"
		...
		case "li_2line_empty":
			return (
				...
				'</i></td><td class="gTitle">' +
				obj.tt[lang] +
				'</td></tr><tr><td class="gVal ac"></td></tr></table></li>'
			);
		case "li_link":
			return (
				'<li id="' +
				id +
				'"><table><tr><td class="gIcon"><i class="material-icons">' +
				obj.i +
				'</i></td><td class="gTitle">' +
				obj.tt[lang] +
				'</td><td class="gIcon"><i class="material-icons">arrow_forward</i></td></tr></table></li>'
			);
		case "li_radio":
			return (
				'<li id="' +
		...
		case "li_radio_change":
			return (
				...
				obj.tt[lang] +
				'</td></tr><tr><td class="gVal ac">' +
				obj.msg[lang].replace("★", obj.m[init.q[id]][lang]) +
				"</td></tr></table></li>"
			);
		case "li_checkbox":
			if (init.q[id] != undefined && init.q[id] == true) on = "hover";
			...
			return (
				...
				'</i></td><td class="gTitle">' +
				obj.tt[lang] +
				'</td><td style="padding:0 1.4rem"><div class="switch ' +
				on +
				'"><div class="toggle"></div></div></td></tr></table><input type="checkbox"/></li>'
			);
		case "li_2line_checkbox":
			if (init.ColData[id][flag] != undefined && init.ColData[id][flag] == true)
				on = "hover";
			else on = "";
			return (
				'<li id="' +
				...
				'"><table><tr><td rowspan="2" class="gIcon"><i class="material-icons">arrow_right</i></td><td class="gTitle">' +
				obj.tt +
				'</td><td rowspan="2"  style="padding:0 1.4rem"><div class="switch ' +
				on +
				'"><div class="toggle"></div></div><input type="checkbox"/></td></tr><tr><td class="gVal ex">' +
				obj.m[lang] +
				"</td></tr></table></li>"
			);
		case "li_2line_checkbox_normal":
			...
			return (
				...
				'</td><td rowspan="2"  style="padding:0 1.4rem"><div class="switch ' +
				on +
				'"><div class="toggle"></div></div></td></tr><tr><td class="gVal ac">' +
				obj.m[lang] +
				'</td></tr></table><input type="checkbox"/></li>'
			);
		case "li_2btn":
			return (
				'<li style="cursor:default" id="' +
				id +
				'" flag="' +
				flag +
				'"class="listBox"><table><tr><td class="gIcon"><i class="material-icons">arrow_right</i></td><td class="gTitle">' +
				obj.tt +
				'</td><td class="UBtn"><i class="material-icons">arrow_upward</i></td><td style="padding:0 1.4rem"></td><td class="DBtn"><i class="material-icons">arrow_downward</i></td></tr></table></li>'
			);
		case "li_box":
			return (
				'<li class="li_box"><table><tr><td rowspan="2" class="gIcon"><i class="material-icons">' +
				obj.i +
				'</i></td><td class="gTitle">' +
				obj.tt[lang] +
				'</td></tr><tr><td class="gVal ex" style="padding-right:1.4rem; text-align:justify">' +
		...
		case "li_text":
			return (
				'<li class="li_text" style="border:0"><table><tr><td class="gIcon"><i class="material-icons">' +
				obj.i +
				'</i></td><td style="width:100%; padding-right:1.4rem"><div class="inputBox"><input class="inputEff" type="text" placeholder="' +
				obj.m[lang] +
				'" id="' +
				id +
				'"><span class="focus-border"></span></div></td></tr></table></li>'
			);
		case "li_full_btn":
			return (
				'<li class="gTitle sendBtn" style="text-align:center; border-top:solid .1rem rgba(255,255,255,.07)">' +
				obj.tt[lang] +
				"</li>"
			);
		case "li_remove_list":
			return (
				...
				'"><table><tr><td rowspan="2" class="gIcon"><i class="material-icons">arrow_right</i></td><td class="gTitle">' +
				id +
				'</td><td rowspan="2" class="gIcon removeBtn"><i class="material-icons">remove_circle_outline</i></td></tr><tr><td class="gVal ac">' +
				flag +
				"</td></tr></table></li>"
			);
		case "li_pn":
		...
		case "li_text_inbtn":
			return (
				'<li class="li_text" style="border:0"><table><tr><td rowspan="2" class="gIcon"><i class="material-icons">' +
				obj.i +
				'</i></td><td style="width:100%;"><div class="inputBox"><input class="inputEff" type="text" placeholder="' +
				obj.m[lang] +
				'" id="' +
				id +
				'"><span class="focus-border"></span></div></td><td rowspan="2" class="gIcon ft sendBtn"><i class="material-icons">send</i></td></tr></table></li>'
		...
		case "li_color":
			...
			input.addClass(
				"shadow inputEff jscolor {onFineChange:'jsColorUpdate(this)', width:240, height:160, position:'bottom', borderColor:'#212121', insetColor:'#161616', backgroundColor:'#212121'}"
			);
			var li =
				'<li id="' +
				id +
				'" class="li_box"><table><tr><td class="gIcon"><i class="material-icons">' +
				obj.i +
				'</i></td><td class="gTitle">' +
				obj.tt[lang] +
				'</td><td style="padding:0 1.4rem">' +
				input.clone().wrapAll("<div/>").parent().html() +
				"</td></tr></table></li>";
			return li;
		case "li_slider":
			var unit = "%";
			...
			return (
				'<li id="' +
				id +
				'" class="li_box"><table><tr><td rowspan="2" class="gIcon"><i class="material-icons">' +
				obj.i +
				'</i></td><td class="gTitle">' +
				obj.tt[lang] +
				'</td><td rowspan="2" style="padding:0 1.4rem"><input class="shadow" type="range" min="' +
				obj.min +
				'" max="' +
				obj.max +
				'" value="' +
		...
		case "li_file":
			return (
				'<li style="padding:0; text-align:center; border:0"><input type="file" name="uploadFile" accept="image/*" /><button>' +
				obj.tt[lang] +
				"</button></li>"
			);
	}
...
function ui() {
	...
	$('.ac, .tab_title.on, nav[name=settings] i, input[type="range"]').css(
		"color",
		"#" + init.Color.accent
	);
	$(".on_bar, .focus-border, .toggle").css(
		"background",
		"#" + init.Color.accent
	);
	...
	if (init.q.pattern == "cross")
		bg =
			...
			oHexColor(init.Color.pattern, parseFloat(init.Range.pattern / 100)) +
			",transparent .1rem),-webkit-linear-gradient(0," +
			oHexColor(init.Color.pattern, parseFloat(init.Range.pattern / 100)) +
			"," +
			oHexColor(init.Color.navBg, parseFloat(init.Range.navBg / 100)) +
			" .1rem)";
		...
		bg =
			"-webkit-linear-gradient(" +
			oHexColor(init.Color.pattern, parseFloat(init.Range.pattern / 100)) +
			",transparent 0),-webkit-linear-gradient(0," +
			oHexColor(init.Color.pattern, parseFloat(init.Range.pattern / 100)) +
			"," +
			oHexColor(init.Color.navBg, parseFloat(init.Range.navBg / 100)) +
			" .1rem)";
	...
	$("nav[name=main], nav[name=history]").css({
		...
		"background-size":
			parseFloat(init.Range.sizePattern / 10) +
			"rem " +
			parseFloat(init.Range.sizePattern / 10) +
			"rem",
		"background-repeat": "repeat",
	});
	if (init.Range.navBg != 100) $(".btn_wrap").css("background", "transparent");
	if (init.Range.edge != 0) {
		$("nav[name=main], nav[name=history]").css({
			border:
				parseFloat(init.Range.sizeEdge / 10) +
				"rem " +
				init.q.edgeType +
				" " +
				oHexColor(init.Color.edge, parseFloat(init.Range.edge / 100)),
		});
	...
	$("nav[name=main] div[name=More]").css({
		"border-top-right-radius":
			parseFloat((init.q.rd_navTR * init.Range.sizeRadius) / 10) + "rem ",
		"border-bottom-right-radius":
			parseFloat((init.q.rd_navBR * init.Range.sizeRadius) / 10) + "rem ",
	});
	$("nav[name=main], nav[name=history]").css({
		"border-top-left-radius":
			parseFloat((init.q.rd_navTL * init.Range.sizeRadius) / 10) + "rem ",
		"border-top-right-radius":
			parseFloat((init.q.rd_navTR * init.Range.sizeRadius) / 10) + "rem ",
		"border-bottom-left-radius":
			parseFloat((init.q.rd_navBL * init.Range.sizeRadius) / 10) + "rem ",
		"border-bottom-right-radius":
			parseFloat((init.q.rd_navBR * init.Range.sizeRadius) / 10) + "rem ",
	});
	if (init.q.applyScope == 1) {
		$(".tableHeader td:first-child").css({
			"border-top-left-radius":
				parseFloat((init.q.rd_tableTL * init.Range.sizeRadiusTable) / 10) +
				"rem ",
			"border-bottom-left-radius":
				parseFloat((init.q.rd_tableBL * init.Range.sizeRadiusTable) / 10) +
				"rem ",
		});
		$(".tableHeader td:last-child").css({
			"border-top-right-radius":
				parseFloat((init.q.rd_tableTR * init.Range.sizeRadiusTable) / 10) +
				"rem ",
			"border-bottom-right-radius":
				parseFloat((init.q.rd_tableBR * init.Range.sizeRadiusTable) / 10) +
				"rem ",
		});
	...
	$(".bar, .mini div").css({
		"border-top-left-radius":
			parseFloat((init.q.rd_graphTL * init.Range.sizeRadiusGraph) / 10) +
			"rem ",
		"border-top-right-radius":
			parseFloat((init.q.rd_graphTR * init.Range.sizeRadiusGraph) / 10) +
			"rem ",
		"border-bottom-left-radius":
			parseFloat((init.q.rd_graphBL * init.Range.sizeRadiusGraph) / 10) +
			"rem ",
		"border-bottom-right-radius":
			parseFloat((init.q.rd_graphBR * init.Range.sizeRadiusGraph) / 10) +
			"rem ",
	});
	...
	var style = "normal";
	if (init.q.time_italic) style = "italic";
	else style = "normal";
	$("[name=time]").css({
	...
	if (init.q.target_italic) style = "italic";
	else style = "normal";
	$("[name=target]").css({
	...
	if (init.q.rps_italic) style = "italic";
	else style = "normal";
	$("[name=rps]").css({
		color: oHexColor(init.Color.rps, parseFloat(init.Range.rps / 100)),
		"font-family":
			"'" + init.q.fRPS + "', 'Roboto Condensed', 'Segoe UI', 'sans-serif'",
	...
	if (init.q.borderTextType == "outline")
		var btt =
			...
			", 0 -.1rem #" +
			init.Color.tableBorderOther;
	else var btt = "0 0 .3rem #" + init.Color.tableBorderOther;
	...
	if (init.q.borderTextType == "outline")
		var btt =
			...
			", 0 -.1rem #" +
			init.Color.tableBorderYOU;
	else var btt = "0 0 .3rem #" + init.Color.tableBorderYOU;
	...
	if (init.q.body_italic) style = "italic";
	else style = "normal";
	$(":not(#YOU).rCell").css({
		background: oHexColor(
			init.Color.view24BgOther,
			parseFloat(init.Range.view24BgOther / 100)
		),
		"border-bottom":
			parseFloat(init.Range.sizeLine / 10) +
			"rem solid " +
			oHexColor(init.Color.tableLine, parseFloat(init.Range.tableLine / 100)),
		height: parseFloat(init.Range.size24TableHeight / 10) + "rem",
	});
	$("#YOU.rCell").css({
		background: oHexColor(
			init.Color.view24BgYOU,
			parseFloat(init.Range.view24BgYOU / 100)
		),
		"border-bottom":
			parseFloat(init.Range.sizeLine / 10) +
			"rem solid " +
	...
	$(".tableHeader td").css({
		background: oHexColor(
			init.Color.tableHd,
			parseFloat(init.Range.tableHd / 100)
		),
		color: oHexColor(
			init.Color.tableHdText,
			parseFloat(init.Range.tableHdText / 100)
		),
	...
	$(".tableBody td:not(:last-child)").css({
		"border-right":
			parseFloat(init.Range.sizeLineVer / 10) +
			"rem solid " +
			oHexColor(
				init.Color.tableLineVer,
				parseFloat(init.Range.tableLineVer / 100)
			),
	});
	$(".tableHeader td:not(:last-child)").css({
		"border-right":
			parseFloat(init.Range.sizeLineVer / 10) +
			"rem solid " +
			oHexColor(init.Color.tableHd, parseFloat(init.Range.tableHd / 100)),
	});
	for (var i in l.size.tab_graph.inner) {
		$("." + i.split("_")[1]).css({
			height: parseFloat(init.Range[i] / 10) + "rem",
			"margin-top":
				parseFloat((init.Range.sizeBody - init.Range[i]) / 10) + "rem",
	...
	$(".tableWrap").css({
		...
		"margin-top":
			parseFloat((init.Range.sizeBody - init.Range.sizeBody) / 10) + "rem",
	});
	...
	if (!init.q.preview24 && init.q.view24_Number != 1 && view == "settings") {
		...
		if ($("#DPSBody_P").length != 0) {
			$("#DPSBody_P").css({
				height:
					parseFloat((tmp * $("#DPSoldBody_P")[0].childElementCount) / 10) +
					"rem",
			});
			$("#HPSBody_P").css({
				height:
					parseFloat((tmp * $("#HPSoldBody_P")[0].childElementCount) / 10) +
					"rem",
			});
	...
	for (var i in init.Order) {
		for (var j in init.Order[i]) {
			...
			if (init.q.header_italic) style_header = "italic";
			else style_header = "normal";
	...
	if (lastCombat != null && String(lastCombat.isActive) == "false") {
		...
		$(".Class, .rIcon").on({
			...
			click: function () {
				...
				else $(this).parent().find(".name").toggleClass("hidden");
			},
		});
	} else $(".Class, .rIcon").css("cursor", "default");
	$(".rRow .rCell").css(
		"width",
		parseFloat(100 / init.Range.size24TableSlice) + "%"
	);
